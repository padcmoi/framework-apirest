...
/**
 * Librairie prise sur https://github.com/PHPMailer/PHPMailer
 */
class PHPMailer_Main
{
...
&& ((PHP_VERSION_ID >= 70000 && PHP_VERSION_ID < 70017) || (PHP_VERSION_ID>= 70100 && PHP_VERSION_ID < 70103)) &&
        ini_get('mail.add_x_header')==='1' && stripos(PHP_OS, 'WIN' )===0) {trigger_error('Your version of PHP is
        affected by a bug that may result in corrupted messages.'
        ...
        if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {throw new Exception($this->
            ...
            */
            ...
            for ($i = 0; $i < $length; ++$i) {$c=$string[$i];if (!ctype_alnum($c) && strpos('@_-.', $c)===false) {
                return false;}}return true;}protected static function isPermittedPath($path) {return
                !preg_match('#^[a-z]+://#i', $path);}protected function mailSend($header, $body) {
                        ...
                        if ($length <= 0) {break;}$len=$length;if ($is_utf8) {$len=$this->utf8CharBoundary($word,
                            ...
                            */
                            ...
                            if ($dec < 128) {if ($encodedCharPos>
                                ...
                                $maxLength -= $lookBack - $encodedCharPos;
                                $foundSplitPos = true;
                                } elseif ($dec < 192) {$lookBack +=3;}} else { $foundSplitPos=true;}}return
                                    $maxLength;}public function setWordWrap() {if ($this->WordWrap < 1) {return;}switch
                                        ($this->
                                        message_type) {
                                        case 'alt':
                                        case 'alt_inline':
